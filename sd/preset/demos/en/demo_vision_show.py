...
n = 0
#m = 0
lcd_draw_string(canvas,50,20, "A Record", color=(255,255,255), scale=2, mono_space=False)
lcd_draw_string(canvas,50,120, "B Finish", color=(255,255,255), scale=2, mono_space=False)
lcd_draw_string(canvas,50,210, "C Show", color=(255,255,255), scale=2, mono_space=False)
...
while True:
    ...
    if _gp_side_a.value() == 1:
        ...
        lcd_draw_string(canvas,70,100, "act"+(str(n+1)), color=(255,255,255), scale=2, mono_space=False)
        lcd.display(canvas, oft=(_canvas_x,_canvas_y))
        time.sleep_ms(20)
        canvas.clear()
        n = n + 1        
    if _gp_side_b.value() == 1:
        ...
        lcd_draw_string(canvas,40,100, "Ready", color=(255,255,255), scale=2, mono_space=False)
        lcd.display(canvas, oft=(_canvas_x,_canvas_y))
        time.sleep_ms(20)
        canvas.clear()
    ...
    if _gp_side_c.value() == 1:
        ...
        lcd_draw_string(canvas,40,100, "Action", color=(255,255,255), scale=2, mono_space=False)
        lcd.display(canvas, oft=(_canvas_x,_canvas_y))
        time.sleep_ms(20)
        canvas.clear()   
